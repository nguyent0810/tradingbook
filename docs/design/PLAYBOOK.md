...
**Known, tracked exception:** `--cd-*` (defined in `globals.css`, backing `.cd-auth`/`.cd-landing`/shell surfaces) is a **second, largely-independent token set** — not just aliasing, but distinct literal values for background/surface/text/spacing/radius/motion-duration (e.g. `--cd-bg:#060a14` vs. `--bg-primary:#09090b`; `--cd-radius:14px` vs. `--radius-lg:12px`). Some values already derive correctly (`--cd-cyan: var(--accent-hover)`, `--cd-focus: var(--accent-hover)`, `--cd-glow-cyan: var(--shadow-glow-accent)`) — that pattern is the target for the rest. This is tracked technical debt (see the consistency-pass batch plan); do not silently merge values that are only *close*, since several are genuinely different by design intent and merging them is a visual change requiring explicit review, not a token rename.
