/**
 * Read-only: build reviewed additive <current active>→500 expansion cohort JSON.
 * No DB writes. Does not activate symbols. Same selection rules as
 * prepare-expansion-300-cohort.ts, but the additive target is computed live
 * from the current active count (post Tier A + Tier B activation) instead of
 * a hardcoded baseline, since the 300-cohort's Tier A/B are already active.
 *
 *   SMOKE_DATABASE=production npx tsx scripts/prepare-expansion-500-cohort.ts
 *   SMOKE_DATABASE=production npx tsx scripts/prepare-expansion-500-cohort.ts --out=data/expansion-500-cohort.json
 */
...
const TARGET_ACTIVE = 500;
const MAX_WEEKDAY_STALE = 60;

/** Already handled by the 206->300 cohort; excluded again here defensively. */
const PRIOR_COLD_START_TRACK = new Set(["HCM", "VCI", "DIG", "DXG", "KBC"]);
...
function parseOut(argv: string[]): string {
  ...
  return resolve(root, "data", "expansion-500-cohort.json");
}
...
async function main(): Promise<void> {
  ...
  console.error("prepare-expansion-500-cohort.ts → DATABASE_URL:", describeDatabaseUrl());
  ...
  const additiveCount = TARGET_ACTIVE - baselineActiveSymbols.length;
  if (additiveCount <= 0) {
    console.error(
      `Live active count ${baselineActiveSymbols.length} already >= target ${TARGET_ACTIVE}. Nothing to do.`
    );
    process.exit(1);
  }
  console.error(
    `Live active baseline: ${baselineActiveSymbols.length}. Additive target for this cohort: ${additiveCount}.`
  );

  const exclusions: Exclusion[] = [];

  for (const sym of PRIOR_COLD_START_TRACK) {
    exclusions.push({
      symbol: sym,
  ...
  for (const sym of allSymbols) {
    ...
    if (sym.active) {
      if ([...PRIOR_COLD_START_TRACK, ...Object.keys(MANUAL_EXCLUDE)].includes(key)) {
        exclusions.push({
          symbol: key,
          reason: "Already active — not additive",
    ...
    const sessionAligned = latest != null && utcDayOnly(latest).getTime() === expectedMs;
    const weekdaySessionsStale =
      latest != null ? countWeekdaysInclusive(latest, expected) - 1 : null;

    if (barCount === 0) {
      if (!PRIOR_COLD_START_TRACK.has(key)) {
        exclusions.push({
          symbol: key,
    ...
    if (PRIOR_COLD_START_TRACK.has(key) || MANUAL_EXCLUDE[key]) continue;
  ...
  const tierA = candidates
    .filter((c) => c.avgValue20Vnd != null && c.avgValue20Vnd >= TRADABILITY_MIN_AVG_VALUE_VND_20)
  ...
  for (const c of tierBPool) {
    if (additivePicked.length >= additiveCount) break;
    additivePicked.push({ ...c, tier: "B" });
  }

  if (additivePicked.length < additiveCount) {
    console.error(
      `Warning: only ${additivePicked.length} additive candidates after filters (wanted ${additiveCount}). ` +
        `Cohort will fall short of the 500 target — see exclusions for why.`
    );
  }

  const additiveSymbols = additivePicked.map((c) => c.symbol);
  const additiveSet = new Set(additiveSymbols);
  if (additiveSet.size !== additiveSymbols.length) {
  ...
  const doc = {
    ...
    policy: {
      ...
      additiveTargetCount: additiveCount,
      additivePickedCount: additivePicked.length,
      intendedActiveCount: baselineActiveSymbols.length + additivePicked.length,
      shortfallFromTarget: TARGET_ACTIVE - (baselineActiveSymbols.length + additivePicked.length),
      batchB1: "NO-GO",
      wholesaleCurateApply: "NO-GO",
      priorColdStartTrack: [...PRIOR_COLD_START_TRACK].sort(),
    ...
    reviewWarnings: [
      "All additive symbols require session backfill (staleness computed at audit time, not post-backfill).",
      ...
      "This cohort was generated after the 300-cohort's Tier A+B were already active — additive symbols here are a disjoint, previously-excluded pool (mostly zero-bar, never fetched before).",
    ],
  };
    ...
    JSON.stringify(
      {
        ...
        shortfallFromTarget: doc.policy.shortfallFromTarget,
