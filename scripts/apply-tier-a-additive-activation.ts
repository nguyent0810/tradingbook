/**
 * Tier A-only additive activation: sets active=true for approved Tier A symbols only.
 * Never deactivates baseline or other rows.
 *
...
async function main(): Promise<void> {
  ...
  const apply = process.env.APPLY_TIER_A_ADDITIVE_ACTIVATION === "1";
  ...
  console.error("[apply-tier-a-additive-activation] DATABASE_URL:", describeDatabaseUrl());
  console.error(JSON.stringify({ activationFile, apply, tierACount: activation.tierASymbols.length }, null, 2));
  ...
  const toActivate = validation.perSymbol.filter((r) => !r.active).map((r) => r.symbol);
  ...
  if (!apply) {
      ...
      JSON.stringify(
        {
          ...
          projectedActiveCount: validation.afterActiveCountIfApplied,
    ...
    console.error(
      `Dry-run only. Set APPLY_TIER_A_ADDITIVE_ACTIVATION=1 to activate ${toActivate.length} Tier A symbol(s).`
    );
    await prisma.$disconnect();
    return;
  }
