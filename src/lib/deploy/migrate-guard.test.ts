...
describe("prod migrate guard — resolveNonPooledUrl", () => {
  it("returns empty when no non-pooled URL is set", () => {
    expect(resolveNonPooledUrl({})).toBe("");
  });
  it("prefers DIRECT_URL, then DATABASE_URL_UNPOOLED, then POSTGRES_URL_NON_POOLING", () => {
    expect(resolveNonPooledUrl({ DIRECT_URL: "postgres://d/x", DATABASE_URL_UNPOOLED: "postgres://u/x" })).toContain("//d/");
    expect(resolveNonPooledUrl({ DATABASE_URL_UNPOOLED: "postgres://u/x" })).toContain("//u/");
    expect(resolveNonPooledUrl({ POSTGRES_URL_NON_POOLING: "postgres://n/x" })).toContain("//n/");
  });
  it("appends sslmode=require and connect_timeout when missing (Neon cold-start safety)", () => {
    const u = resolveNonPooledUrl({ DATABASE_URL_UNPOOLED: "postgres://h/db" });
    expect(u).toMatch(/sslmode=require/);
    expect(u).toMatch(/connect_timeout=30/);
  });
  it("does not duplicate params already present", () => {
    const u = resolveNonPooledUrl({ DATABASE_URL_UNPOOLED: "postgres://h/db?sslmode=require&connect_timeout=10" });
