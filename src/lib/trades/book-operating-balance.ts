...
export function deriveBookOperatingBalanceLines(
  ...
  if (
    ...
    lines.push("Defensive / high-attention posture dominates the open book.");
  } else if (
    activeOpenCount >= 3 &&
    defHaCluster >= Math.ceil(activeOpenCount * 0.4)
  ) {
    lines.push("Clusters skew defensive / high-attention vs stable reviewed.");
  }
  ...
  if (
    ...
    lines.push("Healthy balance — most rows in stable reviewed cluster.");
  }
  ...
  if (
    ...
    lines.push("Stable reviewed cluster shrank vs last visit.");
  }
  ...
  if (
    ...
    lines.push(
      `Pressure concentrated in ${concentration.top1Symbol} + ${concentration.top2Symbol} (planned at-risk sum, where computable).`
    );
  }
