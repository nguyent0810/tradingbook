  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-7">
        <div className="card p-3 relative group cursor-help transition-all hover:border-[var(--accent)]/50">
          ...
          <div
            ...
            style={{ color: expectancy >= 0 ? "var(--pnl-positive)" : "var(--danger)" }}
          >
            {formatVND(expectancy, true)}
          </div>
          {/* Tooltip */}
          <div className="absolute z-30 bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg shadow-xl text-[11px] leading-relaxed text-[var(--text-secondary)] font-normal opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-200 ease-out transform translate-y-1 group-hover:translate-y-0">
            <strong className="text-[var(--text-primary)] block mb-1 font-semibold">System Expectancy (Kỳ vọng)</strong>
            Average net value generated per trade. A positive expectancy confirms a valid mathematical edge.
            <span className="text-[var(--success)] block mt-1.5 font-medium">💡 EOD Action: Continue execution if &gt;0. If negative, halt risk and audit setup criteria.</span>
          </div>
        </div>
        ...
        <div className="card p-3 relative group cursor-help transition-all hover:border-[var(--accent)]/50">
          ...
          {/* Tooltip */}
          <div className="absolute z-30 bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg shadow-xl text-[11px] leading-relaxed text-[var(--text-secondary)] font-normal opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-200 ease-out transform translate-y-1 group-hover:translate-y-0">
            <strong className="text-[var(--text-primary)] block mb-1 font-semibold">Profit Factor (Hệ số LN)</strong>
            Ratio of gross profits to gross losses. Healthy systems target &gt; 1.5.
            <span className="text-[var(--accent)] block mt-1.5 font-medium">📈 EOD Action: Target &gt; 1.5. If below 1.0, audit exits to cut losses faster.</span>
          </div>
        </div>
        ...
        <div className="card p-3 relative group cursor-help transition-all hover:border-[var(--accent)]/50">
          ...
          {/* Tooltip */}
          <div className="absolute z-30 bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg shadow-xl text-[11px] leading-relaxed text-[var(--text-secondary)] font-normal opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-200 ease-out transform translate-y-1 group-hover:translate-y-0">
            <strong className="text-[var(--text-primary)] block mb-1 font-semibold">Max Drawdown (Sụt giảm tối đa)</strong>
            The largest peak-to-trough drop in equity from the historical high watermark.
            <span className="text-[var(--pnl-negative)] block mt-1.5 font-medium">⚠️ EOD Action: If drawdown nears limit, downsize position sizes by 50% immediately.</span>
          </div>
        </div>
